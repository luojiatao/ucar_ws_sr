...
def recognize_and_count(img_path, area):
    ...
    if area == "E":
        ...
        darknet_data.C_cuv += result["cuv"]
        darknet_data.C_riv += result["riv"]
        darknet_data.C_whv += result["whv"] 
        darknet_data.C_cov += result["cov"]
    elif area == "B":
        darknet_data.B_cuv += result["cuv"]
        darknet_data.B_riv += result["riv"]
        darknet_data.B_whv += result["whv"] 
        darknet_data.B_cov += result["cov"]
    elif area == "F":
        darknet_data.F_cuf +=(result["cuf1"] + result["cuf2"]*2 + result["cuf3"]*3)
        darknet_data.F_cof += (result["cof1"] + result["cof2"]*2)
        darknet_data.F_waf += result["waf"] 
...
def callback(data):
    ...
    if data.data == "begin_yolo": 
        print("已接收begin_yolo信号")
         ...
         # 获得所有图片路径
        for img_path in os.listdir("/home/luo/ucar_ws_sr/src/sr_pkg/darknet"):
            img_paths.append("/home/luo/ucar_ws_sr/src/sr_pkg/darknet/" + img_path)

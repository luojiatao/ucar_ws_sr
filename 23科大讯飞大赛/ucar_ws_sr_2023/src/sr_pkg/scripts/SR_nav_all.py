...
def  capture_image(id):
    ...
    while not rospy.is_shutdown():
        if(os.path.exists("/home/ucar/ucar_ws_sr_sr/src/sr_pkg/darknet/{}.jpg".format(id))):
            break
    rospy.sleep(1) 
...
def darknet_callback(data):
    ...
    for key, value_dict in values.items():
        if key == 'F':
            # 对'F'键进行特别处理，返回一个包含最大值对应的键和值的字典
            darknet_max_results[key] = {max(value_dict, key=value_dict.get): max(value_dict.values())}
        else:
            # 对其他键，只返回最大值对应的键
            darknet_max_results[key] = max(value_dict, key=value_dict.get)

    #E_result、D_result、C_result和B_result都是键，而F_result是一个字典，其中包含最大值对应的键和最大值。
...
def play_number(filepath):
    cmd="play ~/ucar_ws_sr/src/sr_pkg/mp3/number/"+filepath+".mp3"
    os.system(cmd)
def play_crop(filepath):
    cmd="play ~/ucar_ws_sr/src/sr_pkg/mp3/crop/"+filepath+".mp3"
    os.system(cmd)

def distinguish(result):
    while not rospy.is_shutdown():
        if result == 'cof' or result == 'cov':
            play_crop("co") #玉米
            break
        elif result == 'cuf' or result == 'cuv':
            play_crop("cu") #黄瓜
            break
        elif result == 'waf' or result == 'wav':
            play_crop("wa") #西瓜
            break
        elif result == 'whf':
            play_crop("wh") #小麦
...
if __name__ == "__main__":
    ...
    start_game =0
    yaml_read()
    # shutil.rmtree("/home/luo/ucar_ws_sr_sr/src/sr_pkg/darknet",ignore_errors=True)
    # os.mkdir("/home/luo/ucar_ws_sr_sr/src/sr_pkg/darknet")
    # # msg_sub()
...
#A
    ...
    # cmd = "play ~/ucar_ws_sr/src/sr_pkg/mp3/done.mp3"
    # os.system(cmd)
    # cmd = "play ~/ucar_ws_sr/src/sr_pkg/mp3/area/B.mp3"
    # os.system(cmd)
    # distinguish(B_result)
    # cmd = "play ~/ucar_ws_sr/src/sr_pkg/mp3/area/C.mp3"
    # os.system(cmd)
    # distinguish(C_result)
    # cmd = "play ~/ucar_ws_sr/src/sr_pkg/mp3/area/D.mp3"
    # os.system(cmd)
    # distinguish(D_result)
    # cmd = "play ~/ucar_ws_sr/src/sr_pkg/mp3/area/E.mp3"
    # os.system(cmd)
    # distinguish(E_result)
    # cmd = "play ~/ucar_ws_sr/src/sr_pkg/mp3/area/F.mp3"
    # os.system(cmd)
    # distinguish(F_result.keys())
    # play_number(F_result.values())

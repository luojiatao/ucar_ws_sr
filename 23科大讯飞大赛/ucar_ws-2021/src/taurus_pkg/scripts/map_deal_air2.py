...
def show_map_img(msg):
    ...
    range_x=np.array([-1,0.5])*0.8  #C区世界坐标x范围
    range_y=np.array([0.5,2.5])*0.8 #C区世界坐标y范围
    ...
    print(car_pose)
    #if car_pose_dis >=0.3 or math.fabs(car_pose[0,2] - car_pose[1,2])>=60:
    return car_pose
...
def pose_callback(req):
    ...
    if len(result)!=0:
        ...
        car_pose_dis=math.sqrt(((result[0,0:2]-result[1,0:2])**2).sum())
        angle_diff=math.fabs(result[0,2] - result[1,2])
        if angle_diff>=360:
            angle_diff-=360
        if car_pose_dis <=0.3 and angle_diff<=60:
            response.pose1[0]=(result[0][0]+result[1][0])/2
            response.pose1[1] =(result[0][1]+result[1][1])/2
            if result[0][2]<0:
                response.pose1[2] =((180+result[0][2])+result[1][2])/2
            elif result[1][2]<0:
                response.pose1[2] =((180+result[1][2])+result[0][2])/2
            else:
                response.pose1[2]=(result[0][2]+result[1][2])/2
            response.count=1
        else:

		...
		if (businessMsg.msgId == 0x01)
		{
			...
			if (major_mic_id <= 5 && major_mic_id >= 0 && mic_angle <= 360 && mic_angle >= 0)
			{
				...
				if (ret1 == 0 && ret2 == 0)
				{
					...
					//判断唤醒角度为0-180还是180-360
					if (mic_angle <= 180 && mic_angle >= 0)
					{
						word = "goal";
					}
					else
...
int main(int argc, char *argv[])
{
	...
	ros::Publisher Start_move = ndHandle.advertise<std_msgs::String>("/start_move", 10); 
    
    ros::Publisher ready_state = ndHandle.advertise<std_msgs::String>("/ready_state", 1); 

	ros::Subscriber ready_launch = ndHandle.subscribe("/xf_launch", 1000, xfCallback);
	...
	ROS_INFO("Wait for launch !");
    while (launch_flag==false && ros::ok())
	{
		ros::spinOnce()	;
	}	
	ROS_INFO("Start launch xf");
	...
	ss2 << word2;
	state_msg.data = ss2.str();
    ready_state.publish(state_msg);
